...
describe HangpersonGame do
  ...
  describe 'new' do
    it 'takes a parameter and returns a HangpersonGame object' do
      @hangperson_game = HangpersonGame.new('glorp')
      expect(@hangperson_game).to be_an_instance_of(HangpersonGame)
      expect(@hangperson_game.word).to eq('glorp')
      expect(@hangperson_game.guesses).to eq('')
      expect(@hangperson_game.wrong_guesses).to eq('')
    end
  end
  ...
  describe 'displayed word with guesses' do
    ...
    @test_cases.each_pair do |guesses, displayed|
      it "should be '#{displayed}' when guesses are '#{guesses}'" do
        guess_several_letters(@game, guesses.to_s)
